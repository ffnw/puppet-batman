define batman::interface (
  ...
  $interfaces.each | $interface | {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE if add ${interface}" ]
  }
  if $orig_interval != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE orig_interval ${orig_interval}" ]
  }
  if $ap_isolation != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE ap_isolation ${ap_isolation}" ]
  }
  if $bridge_loop_avoidance != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE bridge_loop_avoidance ${bridge_loop_avoidance}" ]
  }
  if $distributed_arp_table != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE distributed_arp_table ${distributed_arp_table}" ]
  }
  if $aggregation != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE aggregation ${aggregation}" ]
  }
  if $bonding != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE bonding ${bonding}" ]
  }
  if $fragmentation != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE fragmentation ${fragmentation}" ]
  }
  if $network_coding != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE network_coding ${network_coding}" ]
  }
  if $multicast_mode != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE multicast_mode ${multicast_mode}" ]
  }
  if $loglevel != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE loglevel ${loglevel}" ]
  }
  if $gw_mode != undef {
    if ($gw_mode == 'client') and ($sel_class != undef) {
      $pre_up + [ "/usr/sbin/batctl -m \$IFACE gw_mode ${gw_mode} ${sel_class}" ]
    } elsif ($gw_mode == 'server') and ($bandwidth != undef) {
      $pre_up + [ "/usr/sbin/batctl -m \$IFACE gw_mode ${gw_mode} ${bandwidth}" ]
    } else {
      $pre_up + [ "/usr/sbin/batctl -m \$IFACE gw_mode ${gw_mode}" ]
    }
  }
  if $routing_algo != undef {
    $pre_up + [ "/usr/sbin/batctl -m \$IFACE routing_algo ${routing_algo}" ]
  }
